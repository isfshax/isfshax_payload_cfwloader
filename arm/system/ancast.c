...
int ancast_init(ancast_ctx* ctx, const char* path)
{
    ...
    ctx->path = path;
    if(f_open(&ctx->file, path, FA_READ)) {
        printf("ancast: failed to open %s (%d).\n", path, errno);
        return errno;
    }

    ctx->size = f_size(&ctx->file);

    u8 buffer[0x200] = {0};
    f_read(&ctx->file, buffer, min(sizeof(buffer), ctx->size), &br);
    f_lseek(&ctx->file, 0);
...
int ancast_load(ancast_ctx* ctx)
{
    ...
    btr = ctx->header_size + ctx->header.body_size;
    if(f_read(&ctx->file, ctx->load, btr, &br) || (br != btr)) {
